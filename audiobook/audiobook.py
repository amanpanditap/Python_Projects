...
book = open('book.pdf', 'rb')    # put the pdf name instead of 'book.pdf'
pdfReader = PyPDF2.PdfFileReader(book)
pages = pdfReader.numPages
print(pages)
speaker = pyttsx3.init()
for page_num in range(pages):
    page = pdfReader.getPage(page_num)   # put page no 
    voices = speaker.getProperty('voices')
    speaker.setProperty('voice', voices[0].id)  # put 0 or 1 to change voice 0:male 1:female
    speaker.setProperty('rate', 150)  # put the letters per minute rate in place og 150
